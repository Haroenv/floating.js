...
export default function floating({
  ...
  const MAX = 200;
  for (let x = 0; x <= MAX; x++) {
    float += `${x * 100 / MAX}%{transform:translate(${Math.sin(x / 10) * 10}vw,${(x * (120 / MAX)) - 10}vh)}`;
  }
  float += '}';
  ...
  for (let i = 0; i < number; i++) {
    ...
    floater.style.cssText = `position:absolute;font-size:${_size}em;transform:translateY(110vh);animation:float ${duration}s linear ${i * Math.random()}s ${repeat} ${direction};margin-left:${Math.random() * 100}vw;`;
