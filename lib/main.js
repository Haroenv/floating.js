...
export default function floating({
  ...
  let float = '@keyframes float{';
  const MAX = 200;
  for (let x = 0; x <= MAX; x++) {
    float += `${x * 100 / MAX}%{transform:translate(${Math.sin(x / 10) * 10}vw,${(x * (-120 / MAX)) + 110}vh)}`;
  }
  float += '}';

  document.getElementById('floating-style').innerHTML = '.float-container{width:100vw;height:100vh;overflow:hidden;position:absolute;top:0;left:0;pointer-events:none}.float-container div *{width:1em;height:1em}' + float;
  ...
  for (let i = 0; i < number; i++) {
    ...
    floater.style.cssText = `position:absolute;left:0;font-size:${_size}em;transform:translateY(110vh);animation:float ${duration}s linear ${i * Math.random()}s ${repeat} ${direction};margin-left:${Math.random() * 100}vw;`;
